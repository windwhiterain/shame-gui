...
//!
//! The scenes live in `tests/common/scenes.rs` (shared with the
//! runnable examples via `#[path]`). Each scene is a separate test binary
//! (`tests/snapshot_*.rs`): winit only allows one `EventLoop` per process,
//! so one process = one scene.
//!
...
impl AppRunner for SnapshotRunner {
...
pub fn compare_snapshot_frame2(name: &str, app: App, text_system: TextSystem) {
...
pub fn compare_snapshot(name: &str, app: App, text_system: TextSystem) {
    let path = format!("{}/actual.png", snap_dir(name));
