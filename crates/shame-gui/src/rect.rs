...
impl Rect {
    ...
    ///
    /// Implemented via the generic CPU/GPU [`rect_to_ndc`](crate::dual::rect_to_ndc)
    /// so the same formula feeds the shader.
    pub fn to_ndc(&self, framebuffer: Vec2u) -> Rect {
        let fb_f = Vec2::new(framebuffer.x as f32, framebuffer.y as f32);
        crate::dual::rect_to_ndc::<Rect, false>(*self, fb_f)
    }
}
