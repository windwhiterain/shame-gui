//! A two-field editor [`Widget`] for `Port<Vec2, S>`
...
fn field_rect(rect: Rect) -> (Rect, Rect) {
    ...
    (
        Rect::new(rect.pos, Vec2::new(half_w, rect.size.y)),
        Rect::new(
            rect.pos + Vec2::new(half_w + 8.0, 0.0),
            Vec2::new(half_w, rect.size.y),
        ),
    )
}

fn text_pos(rect: Rect) -> Vec2 {
    rect.pos + Vec2::new(style::FIELD_PAD_X, style::FIELD_PAD_Y)
}

impl<S: 'static> Widget<S> for Port<Vec2, S> {
    ...
    fn render(
        ...
        ctx.texts.push(
            TextObject::new(data.x.display_text(v.x))
        ...
        ctx.texts.push(
            TextObject::new(data.y.display_text(v.y))
        ...
        match event {
            ...
            InputEvent::KeyDown { key } if data.x.editing || data.y.editing => match key {
                ...
                Key::Enter => {
                    if data.x.editing {
                        let vy = v.y;
                        data.x.commit(|vx| self.write(state, Vec2::new(vx, vy)));
                    } else {
                        let vx = v.x;
                        data.y.commit(|vy| self.write(state, Vec2::new(vx, vy)));
                    }
                    EventResponse::Consumed
                }
                Key::Escape => {
