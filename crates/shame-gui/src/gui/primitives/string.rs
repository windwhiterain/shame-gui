//! A text-input [`Widget`] for `Port<String>`.
        ...
        match event {
            InputEvent::MouseDown { pos, .. } => {
                if rect.contains(*pos) {
                    data.buffer = self.read(arena).clone();
            ...
            InputEvent::KeyDown { key } if data.editing => match key {
                ...
                Key::Enter => {
                    let old = self.read(arena).clone();
                    let new = data.buffer.clone();
                    if new != old {
                        self.write(arena, new);
                    }
                    data.editing = false;
                    EventResponse::Consumed
                }
