//! Numeric editor [`Widget`]s for `Port<f32>` and
//! `Port<u32>`: click to edit, Enter commits, Escape cancels.
                ...
                match event {
                    InputEvent::MouseDown { pos, .. } => {
                        if rect.contains(*pos) {
                            data.buffer = self.read(arena).to_string();
