...
impl<S> ViewportNode<S> {
    ...
    pub fn tab(tabs: Vec<(String, ViewportNode<S>)>) -> Self {
        Self::Tab(TabNode { tabs, active: 0 })
    }

    /// A vertical stack of labelled nodes. Layout uses taffy table_rects.
    pub fn container(children: Vec<(String, ViewportNode<S>)>) -> Self {
...
pub struct ViewportTree<S> {
    /// The root node of the tree.
    pub root: ViewportNode<S>,
}

impl<S> ViewportTree<S> {
    /// Creates a tree with the given root.
    pub fn new(root: ViewportNode<S>) -> Self {
        Self { root }
    }
}
