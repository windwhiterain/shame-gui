...
use crate::graph::{Graph, StateArena};
use crate::gui::Gui;
...
pub trait AppRunner {
    ...
    /// returns events to inject for the next step/frame. In `step()` these
    /// events are processed immediately; in `run()` they are queued for the
    /// next `RedrawRequested` cycle.
    fn after_tick(&mut self, _ctx: &AppContext<'_>) -> Vec<InputEvent> {
        vec![]
    }
...
pub struct AppContext<'a> {
    /// The state arena (read port values, inspect widget data).
    pub arena: &'a StateArena,
    /// The finalized graph (source/render ports, dirty state).
    pub graph: &'a Graph,
    /// The attached GUI, if any.
    pub gui: Option<&'a Gui>,
}
